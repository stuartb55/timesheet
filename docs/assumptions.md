...
- Timed, approved authorised absence can cover core time. A whole-day approved credit suppresses missing-core findings when there is no work. Untimed partial credit contributes minutes to balance but cannot prove core-time coverage.
...
- Service Support rota boot-up eligibility is capped at five minutes by its default profile. Editing the configured allowance deliberately changes that cap. Other profiles default to zero.
